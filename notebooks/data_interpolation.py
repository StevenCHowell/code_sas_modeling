...
import numpy
#sh import Gnuplot
#sh import Gnuplot.PlotItems
#sh import Gnuplot.funcutils


#       INTERPOLATE
#
#       06/08/2008      --     initial coding				: jc
#       02/09/2017      --     copied out of sasie          : sh
#
# LC      1         2         3         4         5         6         7
...
def spline(x, y, nval, yp1, ypn):
    ...
    if(ypn > maxval):
        ...
        qn = 0.5
        un = (3.0 / (x[nval - 1] - x[nval - 2])) * (ypn -
                                                    (y[nval - 1] - y[nval - 2]) / (x[nval - 1] - x[nval - 2]))
...
def splint(xa, ya, y2a, nval, x):
    ...
    a = (xa[khi] - x) / h
    b = (x - xa[klo]) / h
    ny = a * ya[klo] + b * ya[khi] + \
        ((a * a * a - a) * y2a[klo] + (b * b * b - b)
         * y2a[khi]) * (h * h) / 6.0
...
def interpolate(variables, txtOutput):
    ...
    runname, expdata, ofile, io, ioe, dq, maxpoints, plotflag = unpack_variables(
        variables)
...
#	print('> back from splines')
    ...
    if(plotflag == 1):
        graph.plot(
            Gnuplot.Data(
                odata, using='1:2 w p ps 4', title='Original Data'), Gnuplot.Data(
                io_tally, using='1:2 w lp ps 2', title='Interpolated Data'), Gnuplot.Data(
                cut, title='[I(Q)/(std.dev. I(Q))] < 2', using='1:2:3 w yerrorbars'))
